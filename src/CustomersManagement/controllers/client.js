...
import { PaymentModule } from "../../Purchasing/modules/payment";
...
const ClientControllers = {
  ...
  async auth(req, res, next) {
    let { email, password } = req.body,
      lang = req.headers.lang || "ar",
      user = await ClientModule.getByEmail(email.toLowerCase()),
      errMsg = lang == "en" ? "Incorrect password" : "كلمة المرور غير صحيحه";
  ...
  async addMobile2Verifications(req, res, next) {
    let { mobile, countryCode } = req.body,
      id = req.params.id,
      smsToken = getSMSToken(5);
    let isExist = await ClientModule.getByMobileNotVerified(mobile);
    if (isExist.doc) {
      let errMsg =
        req.headers.lang == "en"
          ? "This phone number is already in use!, try different one."
          : "رقم الهاتف مستخدم من قبل, برجاء استخدام رقم اخر";
      return next(boom.badData(errMsg));
    }
    /*  let { doc, err } = await ClientModule.addMobile(id, mobile);
    if (err)
      return next(boom.badData(getErrorMessage(err, req.headers.lang || "ar"))); */

    let { doc, err } = await VerificationsModule.add(
      id,
      smsToken,
      countryCode,
      mobile
    ...
    res.status(201).send({
      isSuccessed: true,
      data: {
        smsToken, // this is just for now, till we use sms third party
      },
      error: null,
    });
  },
  async verifyMobile(req, res, next) {
    ...
    await ClientModule.addMobile(
      id,
      verification.doc.countryCode,
      verification.doc.mobile
    );
  ...
  async getFavProducts(req, res, next) {
    let userId = req.body.authId;

    return res.status(200).send({
      isSuccessed: true,
      data: await ClientModule.getFavProducts(userId),
      error: null,
    });
  },
  ...
  async updateProfile(req, res, next) {
    ...
    if (req.body.mobile) {
      if(!req.body.countryCode){
         let errMsg = lang == "en" ? "Country Code must be send" : "يجب ادخال كود الدولة";
      return next(boom.badData(errMsg));
      }
