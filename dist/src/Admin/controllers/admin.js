...
import { Category, Coupon, Cridit, Provider, Subscription } from "../../middlewares/responsHandler.js";
...
const AdminsController = {
  async add(req, res, next) {
    ...
    if (auth && auth.type != "BOSS") {
      let errMsg = req.headers.lang == "en" ? "Not  Allowed!" : "ليس لديك الصلاحية";
    ...
    let {
      user,
      err
    } = await AdminModule.add(email, name, hashedPass);
  ...
  async addCoupon(req, res, next) {
    ...
    if (!provider) {
      let errMsg = req.headers.lang == "en" ? "Provider not found" : "مقد الخدمة غير موجود";
      return next(boom.notFound(errMsg));
    }
    ...
    if (req.file) {
      ...
      Jimp.read("Coupons-Images/" + req.file.filename).then(tpl => {
        return Jimp.read("assets/images/logo.png").then(logoTpl => {
          logoTpl.opacity(0.4);
          ...
          return tpl.composite(logoTpl.resize(logoW, logoH), tpl.bitmap.width - logoW - diffWidth, tpl.bitmap.height - logoH - diffHight, [Jimp.BLEND_DESTINATION_OVER]);
        }).then(tpl => {
    ...
    if (savedCoupon.err) return next(boom.badData(getErrorMessage(savedCoupon.err, req.headers.lang || "ar")));
    savedCoupon = new Coupon(savedCoupon.doc);
    await NotificationModule.newCouponNotification(savedCoupon.id, req.headers.lang, savedCoupon.provider.name);
    return res.status(201).send({
      isSuccessed: true,
      data: savedCoupon,
      error: null
  ...
  async updateProvider(req, res, next) {
    ...
    if (req.file) {
      let logoURL = "/providers-management/providers/providers-images/" + req.file.filename;
    ...
    if (newData.cities) {
      for (let i = 0; i < newData.cities.length; i++) {
        for (let j = 0; j < newData.cities[i].locations.length; j++) {
          let geoInfoAr = await GeoInfoAr.reverseLocation(newData.cities[i].locations[j].lat, newData.cities[i].locations[j].long);
          let geoInfoEn = await GeoInfoEn.reverseLocation(newData.cities[i].locations[j].lat, newData.cities[i].locations[j].long);

          if (geoInfoAr.err || geoInfoEn.err) {
            let errMes = req.headers.lang == "en" ? "Invalide Location" : "الموقع غير صحيح";
          ...
          newData.cities[i].locations[j] = Object.assign(newData.cities[i].locations[j], geo);
        }
      }
    }
    ...
    let {
      doc,
      err
    } = await ProviderModule.updateProvider(id, newData);
    if (err) return next(boom.badData(getErrorMessage(err, req.headers.lang || "ar")));
    console.log("doc: ", doc);
    return res.status(200).send({
      isSuccessed: true,
  ...
  async deleteProvider(req, res, next) {
    ...
    if (coupons.length > 0) {
      console.log(coupons);
      let errMsg = req.headers.lang == "en" ? "Cann't remove this Provider!" : "لا يمكن حذف هذا المستخدم";
      return next(boom.unauthorized(errMsg));
    }
    ...
    let {
      doc,
      err
    } = await ProviderModule.delete(id);
    if (err) return next(boom.badData(getErrorMessage(err, req.headers.lang || "ar")));
    console.log("doc: ", doc);
    return res.status(200).send({
      isSuccessed: true,
  ...
  async updateCategory(req, res, next) {
    ...
    if (req.files) {
      req.files["selectedImage"] ? selected = "/categories-management/categories-images/" + req.files["selectedImage"][0].filename : "";
      req.files["unselectedImage"] ? unSelected = "/categories-management/categories-images/" + req.files["unselectedImage"][0].filename : "";
      category.images = {
    ...
    if (updateCategory.err) return next(boom.badData(getErrorMessage(updateCategory.err, req.headers.lang || "ar")));
  ...
  async deleteCategory(req, res, next) {
    ...
    if (coupons.length > 0) {
      let errMsg = req.headers.lang == "en" ? "Cann't remove this Category!" : "لا يمكن حذف هذا التصنيف";
      return next(boom.unauthorized(errMsg));
    }
  ...
  async toggleProviders(req, res, next) {
    ...
    if (!provider) {
      let errMsg = req.headers.lang == "en" ? "Provider not found" : "مقدم الخدمة غير موجود";
      return next(boom.unauthorized(errMsg));
    }

    if (!provider.isActive && !provider.qrURL) {
      let fileName = provider.code + Date.now() + ".png";
      let qrURL = QRCode.toFile("./Providers-Images/" + fileName, provider.code, {
        color: {
          dark: "#575757",
          // Blue dots
      ...
      provider.qrURL = "/providers-management/providers/providers-images/" + fileName;
    }
  ...
  async passReq(req, res, next) {
    ...
    let user = cardenality.includes("@") ? await ProviderModule.getByEmail(cardenality) : await ClientModule.getByMobile(cardenality);

    if (!user || user.isSocialMediaVerified) {
      let errMsg = req.headers.lang == "en" ? "User not Found!" : "المستخدم غير مسجل";
      return next(boom.notFound(errMsg));
    }

    let smsToken = getSMSToken(5);
    let addVerification = await VerificationsModule.add(smsToken, user._id);
    if (addVerification.err) return next(boom.badData(getErrorMessage(addVerification.err, req.headers.lang || "ar")));
    let mailMessage = req.headers.lang == "en" ? "Enter this code to reset password: " : "ادخل هذا الرمز لاعادة تعين كلمة السر: ",
        msg = "";
    console.log(addVerification);

    if (cardenality.includes("@")) {
      sendClientMail("Reset Password Code", mailMessage + smsToken, cardenality);
      msg = (req.headers.lang == "en" ? "email sent to you, follow it to change your password." : "تم ارسال التعليمات الى بريدك الالكترونى من فضلك قم بأتباعها") + smsToken;
      return res.status(200).send({
        isSuccessed: true,
        data: msg,
        error: null
      ...
      let smsMessage = req.headers.lang == "en" ? "Couponat El-Madena app : welcome to  Couponat El-Madena  your verification code is " : "تطبيق  كوبونات المدينة : مرحبا بك في تطبيق  كوبونات المدينة الرمز التاكيدي لحسابك هو ";
      let sendMess = await Messages.sendMessage(user.countryCode + user.mobile, smsMessage + smsToken);
      console.log("done");
      return res.status(201).send({
        isSuccessed: true,
        data: smsToken,
  ...
  async checkResetCode(req, res, next) {
    ...
    if (verification.err) return next(boom.badData(getErrorMessage(err, req.headers.lang || "ar")));

    if (!verification.doc) {
      let errMsg = req.headers.lang == "en" ? "No previous record for this token!, add phone number or check the token please" : "لا يوجد بيانات سابقه لهذا الرمز فى التحقيقات";
      return next(boom.notFound(errMsg));
    }
    ...
    if (diffInMinuts > 1) {
      ...
      let errMsg = req.headers.lang == "en" ? "This token has exceeded the verification time of 1 minut please resend it" : "رمز التحقيق قد تعدى الوقت المسموح به ( دقيقه واحده ) قم بأعدة ارساله";
      return next(boom.notFound(errMsg));
    }
  ...
  async changePassword(req, res, next) {
    ...
    if (password.length < 8) return next(boom.badRequest("Password must be more than 7 characters"));
    let user = (await ProviderModule.getById(verification.doc.user)) || (await ClientModule.getById(verification.doc.user));
  ...
  async mails(req, res, next) {
    ...
    return res.status(200).send({
      isSuccessed: true,
      data: await ContactModel.find().sort("-createdAt").skip(skip).limit(limit),
      error: null
    });
  },
  ...
  async mailReply(req, res, next) {
    ...
    if (!mail) {
      let errMsg = req.headers.lang == "en" ? "No Mail to reply it" : "لا يوجد رساله للرد عليها";
      return next(boom.notFound(errMsg));
    }
  ...
  async deleteMail(req, res, next) {
    ...
    let {
      doc,
      err
    } = await AdminModule.deleteMail(id);
  ...
  async AdminchangePassword(req, res, next) {
    ...
    if (!user) {
      let lang = req.headers.lang || "ar",
          errMsg = lang == "en" ? "this account not found" : "هذا الحساب غير موجود";
      return next(boom.notFound(errMsg));
    }
  ...
  async getAllSubscriptions(req, res, next) {
    let skip = parseInt(req.query.skip) || null,
        limit = parseInt(req.query.limit) || null;
    let subscriptions = await subscriptionModule.getSubscriptions(null, null, true, true, null, skip, limit);
    ...
    for (let i = 0; i < subscriptions.length; i++) {
      ...
      subscriptions[i].account ? subscriptions[i].account = subscriptions[i].paymentType.key == "ONLINE_PAYMENT" ? await AppCreditModel.findById(subscriptions[i].account + "") : await AppBankModel.findById(subscriptions[i].account + "") : "";
