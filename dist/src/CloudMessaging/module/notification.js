...
let NotificationModule = {
  ...
  async newCouponNotification(coupon, lang, providerName) {
    ...
    for (let i = 0; i < admins.length; i++) {
      if (admins[i].fcmToken && admins[i].fcmToken != "") tokenArray.push(admins[i].fcmToken);
    }

    for (let i = 0; i < users.length; i++) {
      if (users[i].fcmToken && users[i].fcmToken != "") tokenArray.push(users[i].fcmToken);
    }

    for (let i = 0; i < others.length; i++) {
      if (others[i].fcmToken && others[i].fcmToken != "") tokenArray.push(others[i].fcmToken);
    }
    ...
    let message = {
      notification: {
        ...
        body: lang == "en" ? `new Coupon from ${providerName}  , View it ` : `كوبون خصم جديد من ${providerName} , قم بمشاهدته`
      },
      data: {
  ...
  async newProviderNotification(lang, provider) {
    ...
    let message = {
      notification: {
        ...
        body: lang == "en" ? `the Provider ${provider.name} has Registerd newly` : `قام ${provider.name} بالتسجيل كمقدم خدمة`
      },
      data: {
  ...
  async newSubscriptionNotification(lang, provider, couponName, subscription) {
    ...
    let message = {
      notification: {
        ...
        body: lang == "en" ? `Coupon ${couponName.english} has been Purchased newly` : `الكوبون ${couponName.arabic} تم شراءه حديثا`
      },
      data: {
  ...
  async bankTransferNotification(lang, subscription) {
    ...
    let message = {
      notification: {
        title: lang == "en" ? `New Bank Transfer Operation` : `عملية تحويل بنكي جديدة`,
        body: lang == "en" ? `New Payment Operation need to review` : `عملية دفع جديده تحتاج الى مراجعه`
      },
      data: {
        id: JSON.stringify(subscription)
      },
  ...
  async confirmNotification(lang, client, subscription) {
    ...
    let message = {
      notification: {
        ...
        body: lang == "en" ? subscription.decision ? `your payment has been approved` : `your payment has been refused, contatct to admin` : subscription.decision ? `تم قبول عملية التحويل البنكي` : `تم رفض عملية التحويل البنكي برجاء التواصل مع الادمن`
      },
      data: {
    ...
    let saveNotificaion = await NotificationModel({
      ...
      body: {
        english: subscription.decision ? `your payment has been approved` : `your payment has been refused, contatct to admin`,
        arabic: subscription.decision ? `تم قبول عملية التحويل البنكي` : `تم رفض عملية التحويل البنكي برجاء التواصل مع الادمن`
      },
  ...
  async couponUsedNotification(lang, client, couponName, subscription) {
    ...
    let message = {
      notification: {
        title: lang == "en" ? `Subscription Tracking` : `الاشتراكات`,
        body: lang == "en" ? `Coupon ${couponName.english} has been used` : `الكوبون ${couponName.arabic} تم استخدامه حديثا`
      },
      data: {
        id: JSON.stringify(subscription)
      },
  ...
  async getNotifications(userId, type, skip = null) {
    ...
    if (type && type == "CLIENT") return await NotificationModel.find({
      user: {
        $in: [userId, "ALL"]
      }
    }).sort("-createdAt").limit(10);else if (type && type == "ADMIN") return await NotificationModel.find({
      user: {
        $in: ["ADMIN", "ALL"]
      }
    }).sort("-createdAt").limit(10).skip(skip);else return await NotificationModel.find({
      user: userId
