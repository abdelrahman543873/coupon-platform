...
class Coupon {
  constructor(coupon, insideSubscription = null) {
    if (coupon) {
      ...
      !insideSubscription ? this.provider = new Provider(coupon.provider) : "";
...
class Client {
  constructor(client) {
    if (client) {
      ...
      this.imgURL = client.imgURL ? client.isSocialMediaVerified ? client.imgURL : IP + client.imgURL : "";
    }
  }
...
class Subscription {
  constructor(subscription, type, insideSubscription = null) {
    if (subscription) {
      ...
      subscription.account ? this.account = subscription.paymentType.key == "ONLINE_PAYMENT" ? new Cridit(subscription.account) : new Bank(subscription.account) : "";
...
export { Provider, City, Category, Coupon, Client, Bank, Payment, Cridit, Subscription };