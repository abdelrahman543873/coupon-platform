...
const CouponValidations = {
  addCoupon: Joi.object({
    ...
    category: Joi.custom(checkMongooseId, "custom validation").required().error(errorsOverride)
  }),
  updateCoupon: Joi.object({
    ...
    totalCount: Joi.number().optional().positive().allow(0).error(errorsOverride),
    category: Joi.custom(checkMongooseId, "custom validation").optional().error(errorsOverride)
  })
};
