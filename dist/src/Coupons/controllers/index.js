...
const CouponController = {
  ...
  async search(req, res, next) {
    ...
    let user = await ClientModule.getById(id); ///console.log(user.favCoupons);
    ...
    for (let i = 0; i < coupons.length; i++) {
      let sub = user ? await subscriptionModule.getUserSubscripe(user.id, coupons[i].id) : null;
  ...
  async updateCoupon(req, res, next) {
    ...
    if (!(body.servicePrice && body.offerPrice) && body.servicePrice && parseInt(body.servicePrice) <= parseInt(coupons.offerPrice) || body.offerPrice && parseInt(body.offerPrice) >= parseInt(coupons.servicePrice)) {
      let errMsg = req.headers.lang == "en" ? "offer price must be less than service price" : "السعر بعد الخصم يجب ان يكون اقل من السعر قبل الخصم";
      return next(boom.notFound(errMsg));
    }

    body.name && !body.name.arabic ? body.name.arabic = coupons.name.arabic : "";
    body.name && !body.name.english ? body.name.english = coupons.name.english : "";
    body.description && !body.description.arabic ? body.description.arabic = coupons.description.arabic : "";
    body.description && !body.description.english ? body.description.english = coupons.description.english : "";
    ...
    if (updateCoupon.err) return next(boom.badData(getErrorMessage(updateCoupon.err, req.headers.lang || "ar")));
  ...
  async deleteCoupon(req, res, next) {
    ...
    if (subscriptions.length > 0) {
      let errMsg = req.headers.lang == "en" ? "Can't delete coupon" : "لا يمكن مسح هذا الكوبون";
      return next(boom.notFound(errMsg));
    }

    let delet = await CouponModule.delete(id);
    if (delet.err) return next(boom.badData(getErrorMessage(err, req.headers.lang || "ar")));
    console.log("doc: ", doc);
    return res.status(200).send({
      isSuccessed: true,
      data: delet.doc,
  ...
  async getAll(req, res, next) {
    ...
    let coupons = await CouponModule.getAll(skip, limit, category, provider, section);
  ...
  async getById(req, res, next) {
    ...
    coupon = coupon[0];
    let sub = user ? await subscriptionModule.getUserSubscripe(user.id, coupon.id) : null;
