...
let subscriptionContoller = {
  async subscripe(req, res, next) {
    ...
    if (!paymentType) {
      let lang = req.headers.lang || "ar",
          errMsg = lang == "en" ? "Payment Type not Found" : "طريقة الدفع غير موجودة";
      return next(boom.notFound(errMsg));
    }
    ...
    if (paymentType.key == "ONLINE_PAYMENT" || paymentType.key == "BANK_TRANSFER") {
      ...
      if (!req.body.transactionId) {
        let lang = req.headers.lang || "ar",
            errMsg = lang == "en" ? "Transaction Id must added" : "يجب ادخال كود عملية الدفع";
        return next(boom.notFound(errMsg));
      }

      if (paymentType.key == "ONLINE_PAYMENT") account = await AppCreditModel.findById(req.body.account);else account = await AppBankModel.findById(req.body.account);

      if (!account) {
        let lang = req.headers.lang || "ar",
            errMsg = lang == "en" ? "account not found" : "حساب التحويل غير موجود";
        return next(boom.notFound(errMsg));
      }

      if (paymentType.key == "BANK_TRANSFER" && !account.isActive) {
        let lang = req.headers.lang || "ar",
            errMsg = lang == "en" ? "account not Active" : "حساب التحويل غير مفعل";
        return next(boom.notFound(errMsg));
      }
    }
    ...
    if (paymentType.key == "BANK_TRANSFER" && !req.file) {
      let lang = req.headers.lang || "ar",
          errMsg = lang == "en" ? "Transaction Image must added" : "يجب ارفاق اثبات عملية الدفع";
      return next(boom.notFound(errMsg));
    }

    paymentType.key == "BANK_TRANSFER" ? subscription.isConfirmed = false : paymentType.key == "ONLINE_PAYMENT" ? subscription.isPaid = true : "";
    subscription.code = nanoid(6);

    if (req.file) {
      let imgURL = "/purchasing-management/subscriptions-images/" + req.file.filename;
    ...
    if (subscripe.err) {
      ...
      return next(boom.badData(getErrorMessage(subscripe.err, req.headers.lang || "ar")));
    }
    ...
    subscripe.doc.account ? subscripe.doc.account = paymentType.key == "ONLINE_PAYMENT" ? await AppCreditModel.findById(subscripe.doc.account) : await AppBankModel.findById(subscripe.doc.account) : "";
    subscripe = new Subscription(subscripe.doc);

    if (paymentType.key == "ONLINE_PAYMENT" || paymentType.key == "CASH") {
      await NotificationModule.newSubscriptionNotification(req.headers.lang, {
        id: subscripe.coupon.provider.id,
        fcmToken: subscripe.coupon.provider.fcmToken || ""
      }, subscripe.coupon.name, subscripe.id);
    } else if (paymentType.key == "BANK_TRANSFER") {
      console.log("Here nowwwwwwwwwww");
      await NotificationModule.bankTransferNotification(req.headers.lang, subscripe.id);
    }
  ...
  async checkSubscription(req, res, next) {
    ...
    if (!subscription) {
      return res.status(404).send({
        isSuccessed: false,
        data: null,
        error: req.headers.lang == "en" ? "Subscription not Found" : "غير مشترك في هذا الكوبون"
      });
    }

    subscription.account ? subscription.account = subscription.paymentType.key == "ONLINE_PAYMENT" ? await AppCreditModel.findById(subscription.account) : await AppBankModel.findById(subscription.account) : "";
  ...
  async getAllSubscriptions(req, res, next) {
    ...
    if (auth.type == "CLIENT") user = id;else provider = id;
    let subscriptions = await subscriptionModule.getSubscriptions(user, provider, null, null, null);
    ...
    for (let i = 0; i < subscriptions.length; i++) {
      ...
      if (user) {
        subscriptions[i].coupon = await addFavProp([subscriptions[i].coupon], user.favCoupons);
        subscriptions[i].coupon = subscriptions[i].coupon[0];
        console.log("subbb:  ", subscriptions[i].coupon);
        console.log("Fav:  ", user.favCoupons);
        let sub = await subscriptionModule.getUserSubscripe(user._id, subscriptions[i].coupon.id);
  ...
  async getUnconfirmed(req, res, next) {
    let subscriptions = await subscriptionModule.getSubscriptions(null, null, null, false, true, null, null);
  ...
  async confirmPayment(req, res, next) {
    ...
    if (!subscribe || subscribe.isUsed == true) {
      let lang = req.headers.lang || "ar",
          errMsg = lang == "en" ? "Subscription not found" : "عملية الاشتراك غير موجودة";
      return next(boom.notFound(errMsg));
    }

    if ((decision == "false" || decision == false) && !note) {
      let lang = req.headers.lang || "ar",
          errMsg = lang == "en" ? "Must to send note to Client" : "يجب إرسال ملاحظة توضيح سبب الرفض";
      return next(boom.notFound(errMsg));
    }
    ...
    await NotificationModule.confirmNotification(req.headers.lang, {
  ...
  async scan(req, res, next) {
    ...
    let auth = await decodeToken(req.headers.authentication),
        id = auth.id;
    let provider = await ProviderModule.getByCode(code); // let subscribe = await subscriptionModule.scan(id, code);
    ...
    if (subscribtions.length < 1) {
      let lang = req.headers.lang || "ar",
          errMsg = lang == "en" ? `You don't have any subscription for${provider.name} for now` : `أنت غير مشترك في اي كوبون لدى ${provider.name} حالياا`;
      return next(boom.notFound(errMsg));
    }
  ...
  async confirmUsage(req, res, next) {
    ...
    if (!subscribe.isConfirmed) {
      let lang = req.headers.lang || "ar",
          errMsg = lang == "en" ? "Admin Approval is Pending" : "في إنتظار تاكيد الأدمن لعملية الدفع";
      return next(boom.notFound(errMsg));
    }
    ...
    await NotificationModule.couponUsedNotification(req.headers.lang, {
      id: subscribe.user.id,
      fcmToken: subscribe.user.fcmToken || ""
    }, subscribe.coupon.name, subscribe.id);
    return res.status(200).send({
      isSuccessed: true,
      data: subscribe,
      error: null
