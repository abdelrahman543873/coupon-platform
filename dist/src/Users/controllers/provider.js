...
const ProviderControllers = {
  async addProvider(req, res, next) {
    ...
    let logoURL = "";
    provider.email = provider.email.toLowerCase(); // if (req.file) {
    //   logoURL =
    ...
    provider.password = hashedPass; //provider.cities = [...new Set(provider.cities.map((city) => city.id))];

    for (let i = 0; i < provider.cities.length; i++) {
      let city = await CityModule.getById(provider.cities[i].id);

      if (!city) {
        return next(boom.badData(`City with Id ${provider.cities[i].id} not found`));
      }
    }
    ...
    for (let i = 0; i < provider.cities.length; i++) {
      for (let j = 0; j < provider.cities[i].locations.length; j++) {
        let geoInfoAr = await GeoInfoAr.reverseLocation(provider.cities[i].locations[j].lat, provider.cities[i].locations[j].long);
        let geoInfoEn = await GeoInfoEn.reverseLocation(provider.cities[i].locations[j].lat, provider.cities[i].locations[j].long);
        ...
        provider.cities[i].locations[j] = Object.assign(provider.cities[i].locations[j], geo);
      }
    }
    ...
    let {
      doc,
      err
    } = await ProviderModule.add(provider);
  ...
  async updateImage(req, res, next) {
    ...
    let id = (auth.id && auth.type == "PROVIDER" ? auth.id : req.params.id) || null;
    ...
    if (req.file) {
      ...
      let errMsg = req.headers.lang == "en" ? "must add logo" : "يجب اختيار صورة";
      return next(boom.badData(errMsg));
    }
  ...
  async updateProvider(req, res, next) {
    let auth = await decodeToken(req.headers.authentication);
    let id = auth.id;
    let newData = req.body; // if (req.file) {
    //   let logoURL =
    //     "/providers-management/providers/providers-images/" + req.file.filename;
  ...
  async changePassword(req, res, next) {
    ...
    if (!user) {
      let errMsg = req.headers.lang == "en" ? "this account not found" : "هذا الحساب غير موجود";
      return next(boom.badData(errMsg));
    }
    ...
    let {
      doc,
      err
    } = await ProviderModule.changePassword(id, hashedPass);
    if (err) return next(boom.badData(getErrorMessage(err, req.headers.lang || "ar")));
    return res.status(200).send({
      isSuccessed: true,
      data: new Provider(doc),
  ...
  async getAll(req, res, next) {
    let providers = await ProviderModule.getAll(); // providers = providers.map(async (provider) => {
    // });
  ...
  async generatePDF(req, res, next) {
    ...
    if (providers.length < 1) {
      let error = req.headers.lang == "en" ? "No Providers" : "لا يوجود مقدمي خدمات حاليا";
    ...
    let name = id ? stringHash(id) + ".pdf" : "AllProviders.pdf"; //name = name.trim();

    pdfDoc.pipe(fs.createWriteStream("./Providers-Images/" + name));
    pdfDoc.moveDown(25);
    pdfDoc.fillColor("red").font("./assets/fonts/Tajawal-Bold.ttf").fontSize(50) // the text and the position where the it should come
    .text("Couponat El Madina", {
      align: "center"
    });
    providers.map(provider => {
      ...
      let arabic = /[\u0600-\u06FF]/;
      let name = arabic.test(provider.name) ? provider.name.split(" ").reverse().join(" ") : provider.name;
      pdfDoc.fillColor("blue").font("./assets/fonts/Tajawal-Bold.ttf").fontSize(20).text("Provider: ", {
        continued: true
      }).fillColor("black").fontSize(20).text(name, {
        rtl: true
      });
      pdfDoc.moveDown(0.5);
