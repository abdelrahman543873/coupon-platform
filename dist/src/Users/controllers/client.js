...
import { Category, Client, Coupon, Provider } from "../../middlewares/responsHandler.js";
...
const ClientControllers = {
  async add(req, res, next) {
    ...
    if (req.file) {
      imgURL = "/customers-management/customers/customers-images/" + req.file.filename;
    }

    let {
      doc,
      err
    } = await ClientModule.add(name, mobile, countryCode, hashedPassword, imgURL);
    ...
    let sendMess = await Messages.sendMessage(countryCode + mobile, smsMessage + smsToken);
    res.status(201).send({
      isSuccessed: true,
      data: {
        user: client,
  ...
  async home(req, res, next) {
    ...
    for (let i = 0; i < mostSeller.length; i++) {
      let sub = user ? await subscriptionModule.getUserSubscripe(user.id, mostSeller[i].id) : null;
      mostSeller[i].isSubscribe = sub ? true : false;
    }

    for (let i = 0; i < newest.length; i++) {
      let sub = user ? await subscriptionModule.getUserSubscripe(user.id, newest[i].id) : null;
  ...
  async socialMediaRegister(req, res, next) {
    let client = req.body;
    let user = client.mobile ? await ClientModule.getByMobile(client.mobile) : null;

    if (user) {
      let errMsg = req.headers.lang == "en" ? "Mobile Used befor" : "رقم مستخدم من حساب أخر";
    ...
    let {
      doc,
      err
    } = await ClientModule.addViaSocialMedia(client);
    if (err) return next(boom.badData(getErrorMessage(err, req.headers.lang || "ar")));
  ...
  async auth(req, res, next) {
    let {
      ...
      password
        ...
        errMsg = lang == "en" ? "Login cardinalities are invalid!" : "احداثيات المرور غير صحيحه",
        authToken = "";
  ...
  async resendMobileVerification(req, res, next) {
    ...
    if (client.isVerified) {
      let errMsg = lang == "en" ? "Client mobile is already verified" : "جوال المستخدم متوثق من قبل";
      return next(boom.notFound(errMsg));
    }

    let smsToken = getSMSToken(5);
    let deleteCode = await VerificationsModule.delete(id);
    let addToVerificationRes = await VerificationsModule.add(smsToken, client._id);
    if (addToVerificationRes.err) return next(boom.badData(getErrorMessage(addToVerificationRes.err, req.headers.lang || "ar")));
    let smsMessage = req.headers.lang == "en" ? "Couponat El-Madena app : welcome to  Couponat El-Madena  your verification code is " : "تطبيق  كوبونات المدينة : مرحبا بك في تطبيق  كوبونات المدينة الرمز التاكيدي لحسابك هو ";
    let sendMess = await Messages.sendMessage(client.countryCode + client.mobile, smsMessage + smsToken);
    return res.status(200).send({
      isSuccessed: true,
      data: smsToken,
      error: null
  ...
  async updateFavCoupons(req, res, next) {
    ...
    if (!doc) {
      let errMsg = req.headers.lang == "en" ? "Coupon not found" : "كوبون الخصم غير موجود";
      return next(boom.notFound(errMsg));
    }
  ...
  async getFavCoupons(req, res, next) {
    ...
    for (let i = 0; i < coupons.length; i++) {
      let sub = await subscriptionModule.getUserSubscripe(userId, coupons[i].id);
  ...
  async changeMobile(req, res, next) {
    ...
    if (addVerification.err) return next(boom.badData(getErrorMessage(addVerification.err, req.headers.lang || "ar"))); // let smsMessage =
    //   req.headers.lang == "en"
